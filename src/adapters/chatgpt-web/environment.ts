...
function sameTurnAdditionalContextUser(value: unknown, expectedTurnId: string): boolean {
  ...
  if (metadata?.turn_id !== expectedTurnId) return false;
...
/** Codex MultiAgent V2 delivers a child task as agent_message; trust only its direct parent. */
...
/** Parse the current-turn environment claim only; callers must authenticate it against native rollout state. */
...
function canonicalMetadataEnvironmentBeforeUser(
  ...
  while (candidate?.type === "message") {
    ...
    // Codex Work/Desktop can insert same-turn app context (for example writing-block edits)
    // between the canonical environment envelope and the human instruction. These are user-shaped
    // items on the wire, but Codex marks them with a server-owned id plus same-turn
    // internal_chat_message_metadata_passthrough/content_item_kinds. Skip only that narrow
    // provenance shape; an ordinary or forged user message must still break the trust chain.
    if (candidate.role === "user" && sameTurnAdditionalContextUser(candidate, metadataTurnId)) {
      candidateIndex -= 1;
      candidate = record(input[candidateIndex]);
      continue;
    }
...
function rolloutSandboxPolicy(value: unknown, roots: string[]): ChatGptSandboxPolicy | undefined {
  const policy = record(value);
  const rawType = typeof policy?.type === "string" ? policy.type.trim().toLowerCase().replaceAll("_", "-") : "";
  const networkAccess = policy?.network_access === true || policy?.networkAccess === true;
  switch (rawType) {
    case "danger-full-access":
    ...
    case "workspace-write":
    case "workspacewrite":
      return { type: "workspaceWrite", writableRoots: roots, networkAccess };
    case "read-only":
    case "readonly":
...
function rolloutSandboxMatchesMetadata(
  policy: ChatGptSandboxPolicy,
  metadata: Record<string, unknown>,
): boolean {
  const metadataSandbox = sandboxTypeFromMetadata(canonicalSandboxMetadata(metadata));
  if (!metadataSandbox) return false;
  if (metadataSandbox === "platform") return policy.type === "workspaceWrite" || policy.type === "readOnly";
  return metadataSandbox === policy.type;
}

function rolloutCandidateFiles(root: string, identity: string): string[] {
  const results: string[] = [];
  const stack = [root];
  let visited = 0;
  while (stack.length > 0 && visited < 20_000 && results.length < 32) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        stack.push(path);
      } else if (entry.isFile() && entry.name.endsWith(".jsonl") && entry.name.includes(identity)) {
        results.push(path);
...
function rolloutEnvironmentFromFile(
  path: string,
  metadata: Record<string, unknown>,
  tools: CodexTool[],
): ChatGptTurnEnvironment | undefined {
  const explicitSessionId = typeof metadata.session_id === "string" ? metadata.session_id.trim() : "";
  ...
  if (!threadId || !turnId || metadata.parent_thread_id != null || metadata.subagent_kind != null
    || (metadata.agent_name != null && metadata.agent_name !== "/root")) return undefined;
  const expectedSessionId = explicitSessionId || threadId;
  // This local-log bootstrap is intentionally root-thread only. A child/subagent thread must use
  // the separately validated parent-lineage path instead of borrowing a root rollout by session id.
  if (threadId !== expectedSessionId) return undefined;

  let text: string;
  try { text = readFileSync(path, "utf8"); }
  catch { return undefined; }
  // Codex Desktop does not always send session_id in x-codex-turn-metadata. For a root thread the
  // rollout's session_meta id is the canonical thread_id, so that equality can bootstrap the same
  // authority without guessing from process.cwd() or user-authored text. Subagent/thread-spawn
  // ids do not satisfy this root-session equality and continue through the parent-lineage path.
  let sessionMatched = false;
  let currentThreadCwd: string | undefined;
  const environments: ChatGptTurnEnvironment[] = [];
  for (const line of text.split(/\r?\n/)) {
    if (!line || (!line.includes('"session_meta"') && !line.includes('"thread_settings"') && !line.includes('"turn_context"'))) continue;
    ...
    if (event?.type === "session_meta") {
      const eventSessionId = typeof payload.session_id === "string" ? payload.session_id : payload.id;
      sessionMatched = eventSessionId === expectedSessionId;
      continue;
    }
    if (event?.type === "event_msg" && payload.type === "thread_settings_applied") {
      const settings = record(payload.thread_settings);
      const settingsCwd = typeof settings?.cwd === "string" ? settings.cwd : "";
      if (payload.thread_id === threadId && settingsCwd && isAbsolute(settingsCwd)) {
        // Settings are chronological. Keep only the latest value observed before the target
        // turn_context; a later cwd change must never retroactively authenticate an older turn.
        currentThreadCwd = resolve(settingsCwd);
      }
      continue;
    }
    if (event?.type !== "turn_context" || payload.turn_id !== turnId || !sessionMatched) continue;
    const cwdValue = typeof payload.cwd === "string" ? payload.cwd : "";
    const rootValues = Array.isArray(payload.workspace_roots)
      ? payload.workspace_roots.filter((value): value is string => typeof value === "string")
      : [];
    if (!cwdValue || !isAbsolute(cwdValue) || rootValues.length === 0 || rootValues.some(root => !isAbsolute(root))) continue;
    const cwd = resolve(cwdValue);
    if (currentThreadCwd !== undefined && pathIdentity(currentThreadCwd) !== pathIdentity(cwd)) continue;
    const roots = [...new Map(rootValues.map(root => {
      const resolved = resolve(root);
      return [pathIdentity(resolved), resolved] as const;
    })).values()];
    if (!roots.some(root => matchesPath(root, cwd))) continue;
    const policy = rolloutSandboxPolicy(payload.sandbox_policy, roots);
    if (!policy || !rolloutSandboxMatchesMetadata(policy, metadata)) continue;
    const metadataRoots = clientMetadataWorkspaceRoots({ _rawBody: { client_metadata: { "x-codex-turn-metadata": metadata } } as unknown } as CodexParsedRequest);
    if (metadataRoots.length > 0 && !metadataRoots.some(root => matchesPath(root, cwd))) continue;
    environments.push({
      cwd,
      roots,
      writableRoots: policy.type === "dangerFullAccess" || policy.type === "workspaceWrite" ? roots : [],
      sandboxPolicy: policy,
      tools,
    });
  }
  if (!sessionMatched || environments.length === 0) return undefined;
  const fingerprint = (environment: ChatGptTurnEnvironment) => JSON.stringify({
    cwd: pathIdentity(environment.cwd),
    roots: environment.roots.map(pathIdentity).sort(),
    sandbox: environment.sandboxPolicy,
  });
  const unique = new Map(environments.map(environment => [fingerprint(environment), environment]));
  return unique.size === 1 ? [...unique.values()][0] : undefined;
}

function codexRolloutTurnEnvironment(parsed: CodexParsedRequest): ChatGptTurnEnvironment | undefined {
  const metadata = clientTurnMetadata(parsed);
  ...
  const ids = sessionId ? [sessionId, threadId, turnId] : [threadId, turnId];
  if (!ids.every(value => /^[A-Za-z0-9][A-Za-z0-9._-]{7,127}$/.test(value))) return undefined;

  const configuredCodexHome = process.env.CODEX_HOME?.trim();
  const codexHome = resolve(configuredCodexHome || join(homedir(), ".codex"));
  const roots = [join(codexHome, "sessions"), join(codexHome, "archived_sessions")];
  // Native Codex root-thread rollout filenames contain the root session/thread id. Prefer an
  // explicit session_id when present; otherwise the canonical thread_id is the only admissible
  // lookup key and rolloutEnvironmentFromFile must prove session_meta == thread_id.
  const lookupId = sessionId || threadId;
  const files = [...new Set(roots.flatMap(root => rolloutCandidateFiles(root, lookupId)))];
  const tools = parsed.context.tools ?? [];
  const environments = files
    .map(path => rolloutEnvironmentFromFile(path, metadata, tools))
    .filter((environment): environment is ChatGptTurnEnvironment => environment !== undefined);
...
export function extractChatGptTurnEnvironment(parsed: CodexParsedRequest): ChatGptTurnEnvironment {
  ...
  // Codex >=0.153 can omit the historical <environment_context> envelope even though its own
  // rollout turn_context still carries the exact cwd/workspace/sandbox tuple. Use that Codex-owned
  // local record only when there is no cwd declaration at all; conflicting or ambiguous XML must
  // remain fail-closed and can never be rescued by this compatibility path.
  if (cwdMatches.length === 0
    && !/<cwd>[^<]*<\/cwd>/i.test(text)
    && !hasRawChatGptEnvironmentContext(parsed)) {
    const rollout = codexRolloutTurnEnvironment(parsed);
    if (rollout) return rollout;
  }
  return parseChatGptEnvironmentText(parsed, text);
}
...
function parseChatGptEnvironmentText(parsed: CodexParsedRequest, text: string): ChatGptTurnEnvironment {
  ...
  if (!sandboxType) throw new Error("ChatGPT web turn requires one explicit trusted Codex sandbox mode");
...
/** Root tasks have no spawn edge; their canonical rollout must prove the current turn. */
